...
with app.app_context():
    ...
    db.session.commit()
    proj = Project(name='QA Test Project', architect_id=arch.id, client_id=client.id, total_budget=500000, plot_zone='Residential')
    db.session.add(proj)
    db.session.commit()
...
get_arch_csrf() {
    curl -s -b /tmp/arch.cookies http://127.0.0.1:5000/dashboard | grep "csrf-token" | sed 's/.*content="\([^"]*\)".*/\1/' | head -n 1
}
get_client_csrf() {
    curl -s -b /tmp/client.cookies http://127.0.0.1:5000/my_project/ | grep "csrf-token" | sed 's/.*content="\([^"]*\)".*/\1/' | head -n 1
}

echo "Running Login/Auth Tests..."
# Login Architect
CSRF=$(curl -s -c /tmp/arch.cookies http://127.0.0.1:5000/login | grep "csrf_token" | sed 's/.*value="\([^"]*\)".*/\1/')
curl -s -b /tmp/arch.cookies -c /tmp/arch.cookies -d "csrf_token=$CSRF" -d "email=arch@qa.com" -d "password=qapass123" -L http://127.0.0.1:5000/login -o /tmp/arch_home.html
grep -qi "Dashboard\|Logout" /tmp/arch_home.html && echo "✅ 1.2 Architect login successful" || echo "❌ 1.2 Architect login FAILED"

# Login Client
CSRF=$(curl -s -c /tmp/client.cookies http://127.0.0.1:5000/login | grep "csrf_token" | sed 's/.*value="\([^"]*\)".*/\1/')
curl -s -b /tmp/client.cookies -c /tmp/client.cookies -d "csrf_token=$CSRF" -d "email=client@qa.com" -d "password=qapass123" -L http://127.0.0.1:5000/login -o /tmp/client_portal.html
...
REG_CSRF=$(curl -s -c /tmp/reg.cookies http://127.0.0.1:5000/register | grep "csrf_token" | sed 's/.*value="\([^"]*\)".*/\1/')
curl -s -b /tmp/reg.cookies -c /tmp/reg.cookies -d "csrf_token=$REG_CSRF" -d "name=Hacker" -d "email=hacker@qa.com" -d "password=hack123" -d "role=architect" -L http://127.0.0.1:5000/register -o /dev/null
...
curl -s -b /tmp/arch.cookies -c /tmp/arch.cookies -d "csrf_token=$CSRF" -d "name=QA+Project+2" -d "client_id=$CLIENT_ID" -d "plot_zone=Residential" -L http://127.0.0.1:5000/projects/create -o /dev/null
...
# 3.3 Update Budget
CSRF=$(get_arch_csrf)
curl -s -b /tmp/arch.cookies -c /tmp/arch.cookies -d "csrf_token=$CSRF" -d "total_budget=750000" -L "http://127.0.0.1:5000/projects/$PROJECT_ID/payments/budget" -o /dev/null
...
[ "$BUDGET" == "750000.0" ] && echo "✅ 3.3 Update budget successful" || echo "❌ 3.3 Update budget FAILED ($BUDGET)"

# 3.4 Add custom compliance
CSRF=$(get_arch_csrf)
curl -s -b /tmp/arch.cookies -c /tmp/arch.cookies -d "csrf_token=$CSRF" -d "label=Custom+Cert" -L "http://127.0.0.1:5000/projects/$PROJECT_ID/compliance/add-custom" -o /dev/null
...
# 3.5 Toggle
CSRF=$(get_arch_csrf)
curl -s -b /tmp/arch.cookies -c /tmp/arch.cookies -H "X-CSRFToken: $CSRF" -X POST "http://127.0.0.1:5000/projects/$PROJECT_ID/compliance/$COMP_ID/toggle" -o /tmp/toggle.json
...
CSRF=$(get_arch_csrf)
curl -s -b /tmp/arch.cookies -c /tmp/arch.cookies -F "csrf_token=$CSRF" -F "doc=@/tmp/malicious.py" -L "http://127.0.0.1:5000/projects/$PROJECT_ID/documents/upload" -o /tmp/reject.html
...
curl -s -b /tmp/client.cookies -c /tmp/client.cookies -H "X-CSRFToken: $CSRF" -X POST "http://127.0.0.1:5000/projects/$PROJECT_ID/compliance/$COMP_ID/toggle" -o /tmp/toggle_cl.json
...
HEADERS=$(curl -s -I http://127.0.0.1:5000/login)
...
# 5.3 Blind POST
CODE=$(curl -s -b /tmp/arch.cookies -X POST http://127.0.0.1:5000/projects/create -d "name=Hack" -o /dev/null -w "%{http_code}")
